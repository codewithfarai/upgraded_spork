...
async def setup_driver(
    ...
    if license_photo.content_type not in ["image/jpeg", "image/png", "application/pdf"]:
        raise HTTPException(status_code=400, detail="Invalid file type. Only JPEG, PNG, or PDF are allowed.")

    # 2. Upload photo to S3/MinIO
    photo_url = await upload_file_to_s3(license_photo, user_id=auth_id)
    if not photo_url:
