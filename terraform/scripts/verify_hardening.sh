...
# Checks SSH config, Fail2Ban status, and Kernel parameters
...
check_ssh_param() {
    ...
    if grep -qE "^${param}[[:space:]]+${expected}" "$SSH_CONFIG"; then
        ...
        log_fail "SSH: $param is NOT set to $expected (Found: $(grep -E "^${param}" "$SSH_CONFIG"))"
        # If not explicitly set, check if it's default (less reliable, so failing usually safe for hardening)
    fi
}
